...
class MainActivity : AppCompatActivity() , View.OnClickListener{
    ...
    private var cells= mutableListOf<Int,String>()
    ...
    private val combination:Array<IntArray> = arrayOf(
        intArrayOf(0,1,2),
        intArrayOf(3,4,5),
        intArrayOf(6,7,8),
        intArrayOf(0,3,6),
...
if (cells.isNotEmpty()){
    ...
    private fun update(){
        when{
            winner.isNotEmpty()->{
                txtResult.text=resources.getSting(R.string.winner, winner)
    ...
    private fun playGame(index:Int,button: Button){
        ...
        if(!winner.isNullOrEmpty()){
            Toast.makeText(this,"JUEGO FINALZADO SOS UN CRACK",Toast.LENGTH_LONG).show()
            return
        }
